...
int tc_send(int fd, int test_case, int step, int transfer_count, int data_len)
{
    ...
    for (i = 0; i < transfer_count; i++) {
        ...
        if (result != data_len) {
            printf("Sending message was unsuccessful. Send request (tc-step-loop) %d-%d-%d.\r\n",
                test_case, step, i);
            return -1;
        }
    }
    ...
    int data_len)
{
    ...
    for (i = 0; i < transfer_count; i++) {
        ...
        if (result != data_len) {
            printf("Receiving message was unsuccessful. Receive request (tc-step-loop) %d-%d-%d.\r\n",
                test_case, step, i);
            return -1;
        }
        result = pattern_cmp(data, i, data_len);
        if (result == -1) {
            printf("Received bad message. Receive request (tc-step-loop) %d-%d-%d.\r\n",
                test_case, step, i);
            return result;
        }
    }
